...
export default function BusinessControlCenter() {
  ...
  useEffect(() => {
    const unsub = bus.subscribe('inventory:summary-generated', (payload: any) => {
      setInventoryAiStatus((s) => ({ ...s, lastRun: payload?.timestamp }));
    });
    const unsub2 = bus.subscribe('inventory:auto-manage', (payload: any) => {
      setInventoryAiStatus((s) => ({ ...s, autoManage: !!payload?.enabled }));
    });
    return () => {
      unsub();
      unsub2();
        ...
        <Card className="p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-semibold text-gray-900">Inventory AI Agent</h4>
                <p className="text-sm text-gray-600">Autonomous inventory insights and management</p>
              </div>
            </div>
            <div className="text-right text-sm">
              <div>Last run: {inventoryAiStatus.lastRun ? new Date(inventoryAiStatus.lastRun).toLocaleString() : 'Never'}</div>
              <div>Auto-manage: {inventoryAiStatus.autoManage ? 'Enabled' : 'Disabled'}</div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <Button
              onClick={() => {
                try { bus.publish('request:weekly-report'); }
                catch (e) { console.warn(e); }
              }}
              className="bg-indigo-600 text-white"
            >
              Run Weekly Inventory Report
            </Button>

            <Button
              variant="outline"
              onClick={() => {
                try { bus.publish('inventory:auto-manage', { enabled: !(inventoryAiStatus.autoManage) }); }
                catch (e) { console.warn(e); }
              }}
            >
              Toggle Auto-manage
            ...
            <Button variant="outline" onClick={() => navigate('/dashboard')}>Open Inventory Dashboard</Button>
          </div>
        </Card>
